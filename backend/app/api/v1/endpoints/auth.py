...
async def register(
    *,
    ...
    Register new user
    """
    existing_user = await user.get_by_email(db, email=user_in.email)  # ✅ fixed reference
    if existing_user:
        raise HTTPException(
            status_code=400,
            detail="The user with this email already exists."
    ...
    new_user = await user.create(db, obj_in=user_data)  # ✅ fixed reference
    return new_user
