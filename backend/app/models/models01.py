...
# Use standard declarative_base for Alembic compatibility
...
class Address(Base):
    ...
    postal_code = Column(String(32), index=True)
...
class Product(Base):
    ...
    unit_price = Column(Numeric(12,2), nullable=False, index=True)
...
from sqlalchemy.dialects.postgresql import int, JSONB
from sqlalchemy.ext.declarative import declarative_base

Base = declarative_base()

# ---------- Enum definitions ----------
lead_status = Enum("hot", "warm", "cold", name="lead_status")
lead_stage = Enum(
    "lead", "qualified", "proposal", "negotiation", "closed-won", "closed-lost",
    name="lead_stage"
)
ticket_priority = Enum("low", "medium", "high", "urgent", name="ticket_priority")
ticket_status = Enum("open", "in-progress", "resolved", "closed", name="ticket_status")
activity_type = Enum("call", "email", "meeting", "note", "status_change", name="activity_type")
...
class Lead(Base):
    ...
    status = Column(lead_status, server_default=text("'warm'::lead_status"), nullable=False)
    stage = Column(lead_stage, server_default=text("'lead'::lead_stage"), nullable=False)

    value = Column(Numeric(12, 2), server_default=text("0"), nullable=False)
    ...
    notes = Column(Text)
    lead_score = Column(Integer, server_default=text("0"), nullable=False)

    created_at = Column(DateTime(timezone=True), server_default=text("now()"), nullable=False)
    updated_at = Column(DateTime(timezone=True), server_default=text("now()"), onupdate=text("now()"), nullable=False)

    __table_args__ = (
        CheckConstraint("lead_score >= 0 AND lead_score <= 100", name="ck_lead_score"),
        CheckConstraint("value >= 0", name="ck_lead_value_nonnegative"),
        Index("ix_leads_status_stage_assignee", "status", "stage", "assigned_to_employee_id"),
...
class SupportTicket(Base):
    ...
    customer_id = Column(int(as_int=True), ForeignKey("leads.id", ondelete="CASCADE"), nullable=False)
    customer_name = Column(String(255), nullable=False)
    customer_email = Column(String(255), nullable=False)

    priority = Column(ticket_priority, server_default=text("'medium'::ticket_priority"), nullable=False)
    status = Column(ticket_status, server_default=text("'open'::ticket_status"), nullable=False)
    ...
    __table_args__ = (
        ...
        Index("ix_tickets_assignee_status", "assigned_to_employee_id", "status"),
        CheckConstraint("(resolved_at IS NULL) OR (resolved_at >= created_at)", name="ck_ticket_resolved_after_created"),
    )
...
class LeadActivity(Base):
    ...
    employee_id = Column(int(as_int=True), ForeignKey("employees.id", ondelete="SET NULL"))
    ...
    __table_args__ = (
        ...
        Index("ix_lead_activities_scheduled", "scheduled_at"),
        Index("ix_lead_activities_completed", "completed_at"),
        CheckConstraint("duration_minutes IS NULL OR duration_minutes >= 0", name="ck_activity_duration_nonnegative"),
        CheckConstraint(
            "(completed_at IS NULL) OR (scheduled_at IS NULL) OR (completed_at >= scheduled_at)",
            name="ck_activity_completed_after_scheduled"
        ),
    )
...
class TicketComment(Base):
    ...
    author_id = Column(int(as_int=True), ForeignKey("users.id", ondelete="SET NULL"))

    content = Column(Text, nullable=False)
    is_internal = Column(Boolean, server_default=text("false"), nullable=False)
